        ...
        .then((data) => {
            ...
            fetch(`${ENDPOINT}/calculate`, {
                ...
                .then((apiResult) => {
                    ...
                    const resultsValue = parseFloat(apiResult[0]?.eGFR).toFixed(
                        2,
                    );
                    ...
                    fetchUserId()
                        .then((userId) => {
                            ...
                            fetch(`${ENDPOINT}/patient/${userId}/results`, {
                                method: "POST",
                                headers: {
                                    Authorization: `Bearer ${token}`,
                                    "Content-Type": "application/json",
                                ...
                                .then((resultResponse) => {
                                    if (resultResponse) {
                                        console.log(
                                            "Final result sent successfully:",
                                            resultResponse,
                                        );
                                    }
                                    ...
                                    fetch(
                                        `${ENDPOINT}/patient/${userId}/answers`,
                                        {
                                            method: "POST",
                                            headers: {
                                                "Content-Type":
...
function displayResults() {
    ...
    Object.keys(answers).forEach((questionNumber) => {
        ...
        if (questionNumber === "6-Race")
            listItem.innerHTML = `<strong>Question ${4}:</strong> ${answerText}`;
        else if (questionNumber === "7-Height")
            listItem.innerHTML = `<strong>Question ${5}:</strong> ${answerText}`;
        else
            listItem.innerHTML = `<strong>Question ${questionOrder - 1}:</strong> ${answerText}`;
