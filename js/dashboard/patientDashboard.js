    ...
    fetch(`${ENDPOINT}/patient`, {
        ...
        .then((data) => {
            ...
            if (data.results && data.results.length > 0) {
                ...
                document.getElementById("latest-egfr").textContent =
                    `${latest.eGFR} mL/min/1.73m²`;
                document.getElementById("latest-egfr-date").textContent =
                    new Date().toLocaleDateString(); // Using current date as results don't include date

                // Update history
                const historyContainer =
                    document.getElementById("egfr-history");
                sortedResults.slice(1, 3).forEach((result) => {
                    const calculationTypeMatch =
                        result.calculationType.match(/\[(.*?)\]/) ||
                        result.calculationType.match(/\((.*?)\)/);
                    const unit = calculationTypeMatch
                        ? calculationTypeMatch[1]
                        : "mg/dL";
                ...
                historyContainer.innerHTML =
                    '<div class="no-results">No previous results available. Use the eGFR Calculator to get started.</div>';
            }
            ...
            if (appointments.length > 0) {
                const futureAppointments = appointments.filter(
                    (appointment) => {
                        ...
                        let date = new Date(appointment.date).setHours(
                            hours,
                            minutes,
                        );
                        console.log(
                            `appointment date: ${date} current date: ${new Date()}`,
                        );
                ...
                futureAppointments.sort((a, b) => {
                    let dateA = new Date(a.date).setHours(
                        a.time.slice(0, 2),
                        a.time.slice(3, 5),
                    );
                    let dateB = new Date(b.date).setHours(
                        b.time.slice(0, 2),
                        b.time.slice(3, 5),
                    );
