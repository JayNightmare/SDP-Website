...
function displayPatients(patients) {
    ...
    patients.forEach((patient) => {
        ...
        if (
            ...
            const highestResult = patient.results.reduce(
                (max, result) =>
                    result.resultId > max.resultId ? result : max,
                patient.results[0],
            );
    ...
    try {
        ...
        for (const appointment of appointments) {
            try {
                ...
                console.error(
                    `Error fetching patient details for appointment ${appointment.appointmentID}:`,
                    error,
                );
            }
        }
...
async function viewAppointment(appointmentId) {
    try {
        ...
        const patientData = await patientResponse.json();
        const patient = patientData.patients.find(
            (p) => p.patientID === patientData.patientID,
        );
        ...
        document
            ...
            .addEventListener("submit", async (e) => {
                ...
                const errorMessage = document.getElementById(
                    "edit-appointment-error-message",
                );

                try {
                    const updateResponse = await fetch(
                        `${ENDPOINT}/clinician/${clinicianId}/appointments`,
                        {
                            method: "PUT",
                            headers: {
                                "Content-Type": "application/json",
...
async function deleteAppointment(appointment, patientID) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error("Failed to delete appointment from clinician");
...
async function deletePatient(patientID) {
    if (
        !confirm(
            "Are you sure you want to remove this patient? This will also remove all appointments for this patient.",
        )
    )
        return;
    ...
    try {
        ...
        const appointmentsResponse = await fetch(`${ENDPOINT}/clinician`, {
            headers: {
                Authorization: `Bearer ${userToken}`,
            },
        });
        ...
        for (const appointment of patientAppointments) {
            await fetch(`${ENDPOINT}/clinician/${clinicianId}/appointments`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${userToken}`,
...
function contactPatient(patientId) {
    ...
    fetch(`${ENDPOINT}/clinician/${clinicianId}/patients/details`, {
        headers: {
            Authorization: `Bearer ${userToken}`,
            "Content-Type": "application/json",
        },
